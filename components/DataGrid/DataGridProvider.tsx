...
import { DataGridAppearance, DataGridContextValue, DataGridThemeTokens, Density } from './types';
import { resolveThemeVars } from './constants';
...
export const DataGridProvider: React.FC<{ 
  ...
  initialState?: Partial<DataGridContextValue<unknown>['state']>;
  density?: Density;
  appearance?: DataGridAppearance;
  theme?: DataGridThemeTokens;
}> = ({ children, persistenceKey, initialState, density: densityProp, appearance = {}, theme }) => {
  ...
  const paginationPersist = usePersistence<PaginationState>(
    persistenceKey,
    'pagination',
  ...
  const [pagination, setPagination] = useState<PaginationState>(() => 
    persistenceKey ? paginationPersist.load() : (initialState?.pagination || { pageIndex: 0, pageSize: 10 })
  );
  ...
  const value = useMemo(() => ({
    ...
    appearance,
    themeStyle,
  }), [sorting, columnFilters, globalFilter, columnVisibility, rowSelection, pagination, density, appearance, themeStyle]);
...
export const useDataGridContext = <TData,>() => {
